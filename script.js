...
const natureQuotes = [
    "Chaque animal est une œuvre d'art unique ; ensemble, ils forment le chef-d'œuvre de la biodiversité.",
    "La biodiversité est la bibliothèque de la vie. Chaque espèce est un livre irremplaçable.",
    "Observer un animal dans son habitat, c'est contempler la beauté et le mystère à l'état pur.",
    "Protéger une espèce, c'est sauver une couleur de la palette infinie de notre monde.",
    "Des plumes irisées aux fourrures tachetées, la beauté des animaux est une source d'émerveillement infinie.",
    "La richesse de notre planète ne se mesure pas en or, mais en diversité d'espèces vivantes.",
    "Dans le regard d'un animal se trouve toute la splendeur et la fragilité de la vie sauvage.",
    "Notre héritage le plus précieux est la diversité du vivant. Ne la laissons pas disparaître.",
    "L'élégance d'une gazelle, la puissance d'un aigle, la délicatesse d'un papillon : la nature est le plus grand des artistes.",
    "Une planète riche en biodiversité est une promesse d'avenir pour les générations futures.",
    "Chaque maillon de la biodiversité est essentiel. Si l'un disparaît, c'est toute la chaîne qui s'affaiblit.",
    "La nature ne se presse pas, et pourtant tout est accompli. Chaque animal en est la preuve vivante.",
    "Écoutons le chant des oiseaux, admirons la course des fauves. Ce patrimoine vivant est notre responsabilité.",
    "La diversité des formes et des couleurs dans le règne animal est le plus grand spectacle au monde.",
    "Un monde sans la richesse de sa faune serait un jardin sans fleurs, un ciel sans étoiles."
];
...
function getDynamicReproductionLabel(labelKey, category) {
    ...
    <div class="glass-card">
        ...
        `}
                ...
                ${a.conservation ? `<section id="conservation" class="container"><div class="glass-card"><h2>Conservation</h2><div style="text-align:center;"><div class="status-badge" style="background-color:${statusInfo.color};color:${statusInfo.textColor||'#000'};">${statusInfo.name}</div><p>IUCN - ${a.conservation.annee_evaluation}</p></div>${a.conservation.population ? `<div class="stats-grid">${(a.conservation.population.min === 0 && a.conservation.population.max === 0) ? `<div class="stat-box"><span class="stat-number" style="font-size: 1.5rem; line-height: 1.2; font-weight: 600;">Non estimée</span><span class="stat-label">${a.conservation.population.description}</span></div>` : `<div class="stat-box"><span class="stat-number" data-target="${a.conservation.population.max}" data-prefix="${a.conservation.population.min}-">0</span><span class="stat-label">${a.conservation.population.description}</span></div>`}<div class="stat-box stat-box-trend"><span class="stat-number">${a.conservation.tendance === 'Décroissante' ? '↓' : (a.conservation.tendance === 'Croissante' ? '↑' : (a.conservation.tendance === 'Stable' ? '–' : (a.conservation.tendance === 'Inconnue' ? '?' : '→')))}</span><span class="stat-label">${a.conservation.tendance ? `Tendance : ${a.conservation.tendance}` : 'Tendance'}</span></div></div>` : ''}${a.conservation.menaces ? `<h3>Menaces Principales</h3>${a.conservation.menaces.map(m=>`<div class="accordion-item"><div class="accordion-header" onclick="toggleAccordion(this)"><h3>${m.titre}</h3><span class="accordion-icon">▼</span></div><div class="accordion-content"><p>${m.description}</p></div></div>`).join('')}`:''}${a.conservation.mesures ? `<h3>Mesures de Conservation</h3><ul>${a.conservation.mesures.map(m=>`<li>${m}</li>`).join('')}</ul>` : ''}</div></section>` : ''}
                ...
                <footer><p>${natureQuotes[Math.floor(Math.random() * natureQuotes.length)]}</p></footer>`;
        }
    ...
    const targetMax = parseFloat(el.dataset.target); // parseFloat au lieu de parseInt
    if (isNaN(targetMax)) return;
    const prefix = el.dataset.prefix || '';
    const hasRange = prefix.endsWith('-');
    const targetMin = hasRange ? parseFloat(prefix) : null; // parseFloat au lieu de parseInt
    
    // Déterminer le nombre de décimales à afficher
    const decimalsMax = (targetMax.toString().split('.')[1] || '').length;
    const decimalsMin = targetMin !== null ? (targetMin.toString().split('.')[1] || '').length : 0;
    const decimals = Math.max(decimalsMax, decimalsMin);
    ...
    const step = (timestamp) => {
        ...
        const currentMax = (progress * targetMax).toFixed(decimals);
        if (hasRange && targetMin !== null && !isNaN(targetMin)) {
            const currentMin = (progress * targetMin).toFixed(decimals);
    ...
    countersObserver = new IntersectionObserver(e => e.forEach(entry => { 
        if(entry.isIntersecting){ 
    ...
    document.querySelectorAll('.stat-number[data-target]').forEach(c => {
        // Réinitialise le texte à 0
        const prefix = c.dataset.prefix || '';
        c.textContent = prefix.endsWith('-') ? '0-0' : (prefix + '0');
        // Observe l'élément
        countersObserver.observe(c);
    }); 
}
